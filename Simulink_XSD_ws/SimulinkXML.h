...
namespace Simulink
{
  namespace XML
  {
	...
	return b_subsystem_type;
      ...
      const block_connect_list_T& backward (void) const { return b_backward; }
      block_connect_list_T& backward (void) { return b_backward; }
	...
	return b_contain_block;
      ...
      const BlockInfo* action_port (void) const { return b_action_port; }
      BlockInfo*& action_port (void) { return b_action_port; }
    ...
    private:
      ...
      block_connect_list_T b_backward;
      ...
      // trigger port
      BlockInfo* b_trigger_port;
      // enable port
      BlockInfo* b_enable_port;
      // action port
      BlockInfo* b_action_port;
    };
	...
	return r_block_list;
      ...
      const IOPort* find_inport_input (const BlockInfo* port) const;
      IOPort* find_inport_input (const BlockInfo* port);
      const IOPort* find_outport_output (const BlockInfo* port) const;
      IOPort* find_outport_output (BlockInfo* port);
    ...
    protected:
      ...
      void process_top_subsystem (BlockInfo* top, BlockInfo* binfo);
...
extern "C" {
  ...
  c_link_T* get_first_backward (c_block_T* b);
  c_link_T* get_next_backward (c_link_T* i);
