...
BLGraph selectSubsystem(BLGraph graph,string subsystemname);
...
int main(int argc, char *argv[])
{
  ...
  if(argc < 2){
		cout << "usage: ./breadth_first_search <input-XML> [start block list] [LongCycleTask name]" << endl;
		exit(1);
	}
  ...
  /*引数でサブシステムが指定されていればグラフ削減を実行*/
  if(argc == 4){
		graph = selectSubsystem(graph,argv[3]);
	}
  ...
  //for(i = 0;i < node_list_size;i++){
    ...
    //}
  //}
  char startBlockfile[64];
  string str;
  int p;
  //vector<blnode_T*> constantNodeList;
  //vector<string> startNodeList;
  if(argc > 2){
		strcpy(startBlockfile,argv[2]);
		ifstream file(startBlockfile);
		...
		while(getline(file, str)){
			...
			cout << str << endl;
			for(i = 0;i < node_list_size;i++){
				if(str == node_list[i]->p_block->name()){
					node_list[i]->p_block->peinfo("1");
					//cout << str << endl;
					set_id_blgp(node_list[i],0);
					blqueue.push_back(node_list[i]);
				}
			}
		}
		for(i = 0;i < node_list_size;i++){
			if(node_list[i]->p_block->blocktype() == "Constant"){
				node_list[i]->p_block->peinfo("1");
				set_id_blgp(node_list[i],0);
				blqueue.push_back(node_list[i]);
			}
		...
		for(i = 0;i < node_list_size;i++){
			...
			if((node_list[i]->p_in_edge == NULL && node_list[i]->p_out_edge != NULL ) || node_list[i]->p_block->blocktype() == "UnitDelay"){	
				...
				blqueue.push_back(node_list[i]);
			}else if(node_list[i]->p_block->blocktype() == "Constant"){
				//constantブロックの処理を追加するべき
  ...
  vector<blnode_T*> beforeWaitQueue;
  int alreadyCheckFlag = 1;
  for(i = 0;i < (int)blqueue.size();i++){
    current_node = blqueue[i];
	...
	while(!wait_queue.empty()){
		...
		for(j = 0;j < (int)wait_queue.size();j++){
			...
			while(inEdge != NULL){
				...
				if(get_id_blgp(search_node) == -1){
					//cout << "search:" << get_id_blgp(search_node) << search_node->p_block->name() << endl;
					alreadyCheckFlag = 0;
				}
		...
		if(beforeWaitQueue == wait_queue){
			break;
		}
		beforeWaitQueue = wait_queue;
	}
...
////////////////////
// selectSubsystem
// [概要] 入力のBLGraphから指定されたサブシステムのみを抽出
// [引数] BLGraph
// [返り値] 指定されたサブシステムのみのBLGraph
// [備考] 
// 解析を行う前に実行しましょう

BLGraph selectSubsystem(BLGraph graph,string subsystemname)
{
	//指定されたサブシステムをgraph内で検索
	vector<blnode_T*> node_list;
	int node_list_size;
	
	int sNameSize = (int)(subsystemname.size());
	
	node_list = graph.getNodeVector();
	node_list_size = (int)node_list.size();
	
	for(int i = 0;i < node_list_size;i++){
		if(strncmp((node_list[i]->p_block->name()).c_str(),subsystemname.c_str(),sNameSize)){
			graph.deleteNode(node_list[i]);
		}
	}
...
int set_id_blgp(blnode_T *node,int num)
{
  ...
  for(i = 0;i < blgp_size;i++){
    ...
    if(tmp_node->p_block->name() == blk_name){
      ...
      blgp[i].setid1(num);
      cout << "tensai" << node->p_block->name() << endl;
      cout << "mitukatta" << endl;
      return 1;//途中で見つければ成功として1を返す
    }
  }
  cout << "mitukaranai" << endl;
