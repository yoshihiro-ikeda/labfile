...
int main(int argc, char *argv[])
{
  ...
  for(i = 0;i < node_list_size;i++){
    node = node_list[i];
    if(node->p_in_edge == NULL && node->p_out_edge != NULL){
      inport_list.push_back(node->p_block->name());
    }
  }
  
  //場当り的な対処
  inport_list.push_back("ud_mngmdl_sfunc_Block8625");
  ...
  for(i = 0;i < node_list_size;i++){
    node = node_list[i];
    if(node->p_in_edge != NULL && node->p_out_edge == NULL && judge_core_info(node,-1)){
  ...
  //場当り的な対処
  outport_list.push_back("ud_mngmdl_sfunc_Block8625");
  ...
  if(debug_flag == 1){
		...
		cout << "-------------------------------------check----------------------------------------------" << endl;
    for(i = 0;i < (int)inport_list.size();i++){
    ...
    //print_core_info(0,output_filename);
		print_core_info(0,"test_core0.csv");
    ...
    cout << "-------------------------------------check----------------------------------------------" << endl;
    for(i = 0;i < (int)outport_list.size();i++){
...
void confirm_cycle_lag(BLGraph graph,int core,vector<string> sblk_list,vector<string> eblk_list)
{
  ...
  for(i = 0;i < (int)sblk_list.size();i++){		//始点ブロックリストの数だけr_trace_graphを呼び出す
    for(j = 0;j < node_list_size;j++){	//それらのブロック名がnode_list内のブロック名と合致した場合のみ実行
      node = r_node_list[j];
      if(node->p_block->name() == sblk_list[i]){
				if(judge_core_info(node,-1) || node->p_block->name() == "ud_mngmdl_sfunc_Block8625"){	//コア割り当てされていなければコア割り当てを実行しtrace_graphを呼び出し
...
void print_core_info(int flag,char *filename)
{
  ...
  if(output_flag == 0){
    ...
    result << "blockname,,peinfo" << endl;
    if(flag == 1){
      for(i = 0;i<node_list_size;i++){
				if(!judge_core_info(node_list[i],-1)){
					result << node_list[i]->p_block->name() << ",," << node_list[i]->p_block->peinfo() << endl;
				}else{
					result << node_list[i]->p_block->name() << ",," << 4 << endl;
				}
      }
    }else{
      for(i = 0;i<node_list_size;i++){
