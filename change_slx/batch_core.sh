...
read -p "execute taskmapping? y/n:" str
if test $str = "y"
then
    ln -s ../../bltmp2c .
    ./bltmp2c 2 out_"$mdlname".xml tmp_perf.xml
    read -p "execute csp_translator? y/n:" str
    if test $str = "y"
    then
        ln -s ../../../git/TOYOTA/csp_translator/csp_translator .
        ./csp_translator -v -s tmp_perf.xml -i para_"$mdlname"_res.c -T mcos -X para_"$mdlname".c --create-t0=y
    ...
    if test $str = "y"
    then
        ln -s ../../../git/NIS/csp_translator/csp_translator .
